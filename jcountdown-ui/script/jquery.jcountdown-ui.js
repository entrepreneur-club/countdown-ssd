...
$.widget("jui.countdownui", {
	...
	_create: function() {
		...
		var opts = this.options,
			...
			$pFirst = $("<div>").addClass("ui-widget-content ui-state-default jui-first jui-active"),
			$pLast = $("<div>").addClass("ui-widget-content ui-state-default jui-last jui-not-active"),
			testDate;
	...
	_timerFunc: function() {
		...
		if( !settings.hoursOnly ) {
			if( settings.prevDays === undefined ) {
				...
				$divUiDays.find('div.jui-active').html( daysLeft + " " + settings.daysText );
			}
		}
		...
		if( settings.yearsAndMonths ) {
			...
			if( settings.prevYears === undefined ) {
				...
				$divUiYears.find('div.jui-active').html( yearsLeft + " " + settings.yearsText );
			}		

			if( settings.prevMonths === undefined ) {
				settings.prevMonths = monthsLeft;
				$divUiMonths.find('div.jui-active').html( monthsLeft + " " + settings.monthsText );
			}	
		}
		
		if( settings.prevHrs === undefined ) {
			settings.prevHrs = hrsLeft;
			$divUiHours.find('div.jui-active').html( hrsLeft + " " + settings.hoursText );
		}
		if( settings.prevMins === undefined ) {
			settings.prevMins = minsLeft;
			$divUiMinutes.find('div.jui-active').html( '<div class="time">'+minsLeft + '</div> <div class="unit">' + settings.minutesText+'</div>' );
		}
		if( settings.prevSec === undefined ) {
			settings.prevSec = secLeft;
			$divUiSeconds.find('div.jui-active').html( '<div class="time">'+secLeft + '</div> <div class="unit">' + settings.secondsText + '</div>' );
		}
...
$.fn.countdownui.transitions = {	
	turnDown: function( $elem, opts ) {
		...
		var $activeElem = $elem.find('div.jui-active'),	
			$inactiveElem = $elem.find('div.jui-not-active');
...
$.fn.countdownui.animate = function( $elem, opts ) {
	...
	$inactiveElem.html( '<div class="time">'+opts.timeLeft + '</div> <div class="unit">' + opts.timeText + '</div>');
